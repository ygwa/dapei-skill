...
- `repos/` — mapped repos
- `docs/01-08` — eight stage docs: current-state → gap-analysis → solution-design → task-breakdown → implementation → local-validation → architecture-review → acceptance
...
At each confirmation point (`solution-design`, `implementation`, `acceptance`), the AI pauses for your confirmation rather than bulldozing through. The engine enforces the gates via `feature.stage.transition` and `feature.accept`, both of which refuse without `confirmed=true`.
