...
changelog:
  ...
  1.1.0: "增强 repos 分析、context build、guardrail 和 feature 命令"
  1.0.0: "初始稳定版本"
---
...
- 首选唤醒词：`@dapei`
- 当用户消息以 `@dapei` 开头，或用户的意图明确指向 workspace/feature/repos 管理时，进入 dapei 工作模式。
...
| "接入 xxx 代码库" / "把 xxx 加进来" / "分析下 xxx 项目" | repos-manage | `scripts/dapei repos add/analyze` |
...
2. 如果用户没给 URL，询问
3. 执行 `scripts/dapei repos add <name> <url>`
4. 执行 `scripts/dapei repos analyze <name>` 做深度分析
...
2. 如果用户没有明确给出 repos，从 `.dapei/reposs.yaml` 中推荐相关的 repo
...
1. **所有代码变更必须在 `features/<feature>/repos/<repo>/` 下进行**，绝不直接修改 `repos/` 下的文件
...
1. 如果 `docs/` 下缺少关键文档，提醒用户可以通过 `@dapei repos analyze` 生成
...
3. **一级运行目录**必须是 `repos/`、`docs/`、`features/`
