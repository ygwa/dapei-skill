...
test('documentation-contract: SKILL.md references match capability implementations', () => {
  ...
  for (const skillName of readdirSync(SKILLS_DIR)) {
    ...
    for (const ref of capRefs) {
      if (['package.json', 'feature.yaml', 'workspace.yaml', 'repos.yaml', 'repo.yaml', 'pom.xml', 'package-lock.json', 'tsconfig.json', 'go.mod', 'go.sum', 'agents.md', 'README.md', 'index.yaml', 'cognitive.yaml', 'commands.yaml', 'feature.schema.yaml', 'repos.schema.yaml', 'domain.yaml', 'product-map.yaml', 'cross-links.yaml', 'domain-suggestions.yaml'].includes(ref)) continue;
